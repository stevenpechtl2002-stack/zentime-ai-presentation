...
function buildPart1(form, plan, pd, today) {
  return `============================================================================
...
function buildPart2(form, plan, today) {
...
(1) Genehmigte Unterauftragsverarbeiter:
  ...
  +----------------------+------------------------------+------------------+----------------------+
  | ElevenLabs Inc.      | KI-Sprachsynthese &          | USA / SCCs gem.  | Art. 46 Abs. 2       |
  | (Scale-Plan)         | Erkennung – Echtzeit,        | Art. 46 DSGVO    | lit. c DSGVO (SCCs)  |
  |                      | keine dauerh. Speicherung    |                  |                      |
  +----------------------+------------------------------+------------------+----------------------+
  | n8n GmbH             | Workflow-Automatisierung,    | Deutschland / EU | Art. 45 DSGVO        |
  |                      | Anruf-Routing                |                  | (EU-Inland)          |
  +----------------------+------------------------------+------------------+----------------------+
  | Lovable Technologies | Frontend + gesamte           | EU               | Art. 45 DSGVO (EU)   |
  |                      | Supabase-Infrastruktur       |                  |                      |
  +----------------------+------------------------------+------------------+----------------------+
  | Vercel Inc.          | Hosting Landing Page –       | USA / SCCs       | Art. 46 Abs. 2       |
  |                      | nur anonyme Analytics        |                  | lit. c DSGVO (SCCs)  |
  +----------------------+------------------------------+------------------+----------------------+
...
(3) Lovable (EU): Hauptvertragspartner für Backend inkl. Supabase. EU-Verarbeitung.
    Lovable verantwortlich für Backups, Verfügbarkeit und Sicherheit.
...
--------------------------------------------------------------------------------
  +----------------------------------+--------------------------------------+
  | Maßnahme                         | Umsetzung bei Zentime               |
  +----------------------------------+--------------------------------------+
  | Zugangskontrolle                 | Supabase Auth via Lovable: 2FA,     |
  |                                  | rollenbasierte Zugriffsrechte (RBAC)|
  +----------------------------------+--------------------------------------+
  | Verschlüsselung                  | TLS 1.2+ alle Übertragungen;        |
  |                                  | Supabase at-rest AES-256 via Lovable|
  +----------------------------------+--------------------------------------+
  | Datentrennung (RLS)              | Supabase Row Level Security via     |
  |                                  | Lovable: Mandantentrennung pro Kunde|
  +----------------------------------+--------------------------------------+
  | Eingabekontrolle                 | Supabase Audit Logs via Lovable:    |
  |                                  | alle Zugriffe mit Zeitstempel & ID  |
  +----------------------------------+--------------------------------------+
  | Verfügbarkeit                    | Lovable automatische Backups;       |
  |                                  | n8n Fehler-Alerting; Vercel CDN     |
  +----------------------------------+--------------------------------------+
  | Serverstandort                   | Primär EU (Lovable/Supabase         |
  |                                  | Frankfurt); ElevenLabs + Vercel USA |
  |                                  | mit SCCs gemäß Art. 46 DSGVO        |
  +----------------------------------+--------------------------------------+

--------------------------------------------------------------------------------
Art. 8  Datenlösch- und Rückgabepflichten
--------------------------------------------------------------------------------
(1) Nach Vertragsende: Export (Supabase via Lovable, CSV/JSON) oder
    Löschung innerhalb von 30 Tagen auf Anfrage.
(2) Transkriptionen: automatische Löschung nach 90 Tagen (konfigurierbar).
    ElevenLabs-Audiodaten: nicht dauerhaft gespeichert.
(3) n8n-Workflow-Logs: Löschung nach 30 Tagen.
(4) Löschbestätigung schriftlich auf Anfrage. Gesetzliche Aufbewahrungsfristen
    bleiben unberührt.

--------------------------------------------------------------------------------
Art. 9  Kontrollrechte des Verantwortlichen
--------------------------------------------------------------------------------
Kontrolle durch: Nachweise (Lovable/Supabase Audit Logs, n8n-Protokolle,
ElevenLabs-DPA-Kopie), Selbstauskünfte, Audits (4 Wochen Ankündigung,
max. 1x jährlich, Geschäftszeiten). Kosten trägt Verantwortlicher,
außer bei wesentlichen Verstößen.
